...
export function zoomComparisonWindow(current: number, wheelDeltaPixels: number, maximum = TISSUE_COMPARISON_PLAN.durationMilliseconds) {
...
export function comparisonSampleTimes(tissues: readonly ComparisonTissue[], durationMilliseconds = TISSUE_COMPARISON_PLAN.durationMilliseconds) {
  ...
  const times = new Set<number>([PRE_RF_SAMPLE_TIME_MS, 0, durationMilliseconds])
  for (let time = -leadIn; time <= durationMilliseconds; time += plan.sampleIntervalMilliseconds) times.add(time)
  ...
  for (const tissue of tissues) {
    ...
    for (let index = 1; index <= 100; index++) {
      ...
      if (time <= durationMilliseconds) times.add(time)
    }
  }
