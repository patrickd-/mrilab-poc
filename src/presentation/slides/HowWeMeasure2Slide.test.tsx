...
it('starts both graphs at 12 seconds and zooms them together without resetting playback', () => {
  vi.useFakeTimers()
  const { container } = render(<Slide fieldStrengthTesla={1.5} stateIndex={0} direction="forward" setFieldStrengthTesla={() => {}} />)
  finishEntrance()
  ...
  expect(screen.queryByText('Scroll either graph to zoom time')).toBeNull()
  expect(upper.getAttribute('data-time-window-ms')).toBe('12000')
  expect(lower.getAttribute('data-time-window-ms')).toBe('12000')
  expect(screen.getAllByText('12')).toHaveLength(2)
  fireEvent.wheel(upper, { deltaY: -200, deltaMode: 0 })
  const firstWindow = Number(upper.getAttribute('data-time-window-ms'))
  expect(firstWindow).toBeLessThan(12000)
  ...
  expect(screen.getAllByText('t (s)')).toHaveLength(2)
  expect(Number(lower.getAttribute('data-time-window-ms'))).toBeLessThan(firstWindow)
  expect(lower.getAttribute('data-time-window-ms')).toBe(upper.getAttribute('data-time-window-ms'))
  ...
  const beforeKeyboard = Number(upper.getAttribute('data-time-window-ms'))
  fireEvent.keyDown(container.querySelector('.tissue-plots')!, { key: '+' })
  expect(Number(upper.getAttribute('data-time-window-ms'))).toBeLessThan(beforeKeyboard)
  fireEvent.keyDown(container.querySelector('.tissue-plots')!, { key: '-' })
  expect(Number(upper.getAttribute('data-time-window-ms'))).toBeCloseTo(beforeKeyboard)
