...
it('keeps both graphs at 12 seconds without zoom controls while sharing hover time', () => {
  vi.useFakeTimers()
  const { container } = render(<Slide fieldStrengthTesla={1.5} stateIndex={0} direction="forward" setFieldStrengthTesla={() => {}} />)
  finishEntrance()
  ...
  for (const key of ['+', '-', '0']) fireEvent.keyDown(container.querySelector('.tissue-plots')!, { key })
  expect(upper.getAttribute('data-time-window-ms')).toBe('12000')
  expect(container.querySelectorAll('.voltage-trace__tag')).toHaveLength(1)
